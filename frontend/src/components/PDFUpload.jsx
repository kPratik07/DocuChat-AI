...
const PDFUpload = ({
    ...
    async (acceptedFiles, fileRejections) => {
      ...
      if (fileRejections.length > 0) {
        ...
        if (rejection.errors[0]?.code === "file-invalid-type") {
          setFileRejectionError(
            "Only PDF files are allowed. Please upload a PDF document.",
          );
      ...
      try {
        ...
        const pdfData = {
          ...
          url: `${API_BASE_URL}/uploads/${response.data.storedName}`,
        };
        ...
        if (error.response?.status === 413) {
          setErrorMessage(
            "File is too large. Please upload a PDF smaller than 50MB.",
          );
  ...
  return (
    <div className={`max-w-md w-full mx-4 ${className}`}>
      <div
        {...getRootProps()}
        className={`
          bg-white rounded-lg shadow-lg p-8 text-center cursor-pointer transition-all duration-200
          ${isDragActive && !isDragReject ? "border-2 border-[#337b66] bg-[#edf4eb]" : ""}
          ${isDragReject ? "border-2 border-red-400 bg-red-50" : ""}
          ${!isDragActive && !isDragReject ? "border-2 border-dashed border-[#9db9a4]" : ""}
          ${isUploading ? "opacity-50 cursor-not-allowed" : "hover:border-[#337b66] hover:bg-[#edf4eb]"}
        `}
      >
        <input {...getInputProps()} />
        ...
        <div className="mb-6">
          ...
          >
            {uploadStatus === "error" || fileRejectionError ? (
              ...
              <FileText className="w-12 h-12 text-[#236b57]" />
            ) : (
              <Upload className="w-12 h-12 text-[#236b57]" />
            )}
          </div>
        </div>
        ...
        {uploadStatus === "uploading" &&
          ...
          !isDragReject && (
            <>
              <h2 className="text-xl font-semibold text-[#236b57] mb-6">
                Uploading PDF
              </h2>
              ...
              <div className="w-full mb-4">
                <div className="w-full bg-gray-200 rounded-full h-3 overflow-hidden">
                  <div
                    className="bg-[#236b57] h-3 rounded-full transition-all duration-300 ease-out"
              ...
              {/* Percentage */}
              <p className="text-2xl font-bold text-[#236b57] mb-2">
                {progress}%
              </p>
        ...
        {uploadStatus === "idle" && !fileRejectionError && !isDragReject && (
          <>
            ...
            {isDragActive && !isDragReject && (
              <div className="bg-[#edf4eb] border border-[#b8cbb9] rounded-lg p-4 mb-4">
                <p className="text-[#236b57] font-medium">Drop your PDF here</p>
              </div>
            )}
        ...
        {!isUploading && uploadStatus === "idle" && !fileRejectionError && (
          <div className="mt-6">
            <button
              type="button"
              className="inline-flex items-center px-6 py-3 border border-transparent text-base font-semibold rounded-lg text-white bg-[#236b57] hover:bg-[#1b5848] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#337b66] transition-colors"
            >
              Choose PDF File
            </button>
          </div>
